...
function ContactForm() {
  ...
  const onSubmit = (data: ContactFormData) => {
    ...
    // Native browser POST — FormSubmit AJAX is blocked; classic form submit works.
    const form = document.createElement("form");
    form.method = "POST";
    form.action = `https://formsubmit.co/${personalInfo.email}`;
    ...
    add("_subject", `[Portfolio] ${data.subject}`);
    add("_template", "table");
    add("_captcha", "false");
    add("_next", `${window.location.origin}/contact?sent=1`);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-10 items-start">
            <StaggerContainer className="lg:col-span-7 w-full min-w-0">
              <StaggerItem className="w-full min-w-0">
                <Suspense
                  fallback={
                    <Card className="p-4 sm:p-6 md:p-8 w-full min-h-[420px] animate-pulse" />
                  }
                >
                  <ContactForm />
                </Suspense>
